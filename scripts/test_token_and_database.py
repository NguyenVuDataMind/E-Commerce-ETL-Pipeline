...
def test_token_refresh_real():
    ...
    try:
        ...
        try:
            ...
            logger.info(f"Access token: {authenticator.access_token[:20]}..." if authenticator.access_token else "No access token")
            logger.info(f"Refresh token: {authenticator.refresh_token[:20]}..." if authenticator.refresh_token else "No refresh token")
        ...
        logger.info(f"Refresh token expiry status: {'Expired' if is_refresh_expired else 'Valid'}")
        ...
        if is_expired:
            ...
            try:
                ...
                if success:
                    ...
                    logger.info(f"New access token: {authenticator.access_token[:20]}...")
                    logger.info(f"New refresh token: {authenticator.refresh_token[:20]}...")
                else:
    ...
    try:
        ...
        try:
            if hasattr(misa_loader, 'validate_data_format'):
                is_valid = misa_loader.validate_data_format('customers', sample_customers_df)
...
def main():
    ...
    if passed_tests == total_tests:
        ...
        logger.warning(f"⚠️ {total_tests - passed_tests} tests failed. Please check configuration.")
        return 1
