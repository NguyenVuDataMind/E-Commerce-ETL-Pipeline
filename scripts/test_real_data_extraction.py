...
def test_misa_crm_real_extraction():
    ...
    try:
        ...
        if customers_data:
            ...
            logger.info(f"Sample data: {json.dumps(customers_data[0] if customers_data else {}, indent=2)}")
...
def test_tiktok_shop_real_extraction():
    ...
    try:
        ...
        logger.info(f"Testing orders from {start_date.strftime('%Y-%m-%d')} to {end_date.strftime('%Y-%m-%d')}")
        
        # Test với batch size nhỏ
        orders_data = []
        batch_count = 0
        
        for orders_batch in extractor.stream_orders_lightweight(start_timestamp, end_timestamp, batch_size=10):
            orders_data.extend(orders_batch)
            batch_count += 1
            logger.info(f"Batch {batch_count}: {len(orders_batch)} orders (total: {len(orders_data)})")
        ...
        if orders_data:
            logger.info(f"✅ TikTok Shop orders: {len(orders_data)} records from {batch_count} batches")
            logger.info(f"Sample data: {json.dumps(orders_data[0] if orders_data else {}, indent=2)}")
            return True
        else:
...
def main():
    ...
    if passed_tests == total_tests:
        ...
        logger.warning(f"⚠️ {total_tests - passed_tests} tests failed. Please check API configuration.")
        return 1
