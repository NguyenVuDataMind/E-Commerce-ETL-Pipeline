...
def transform_misa_crm_data(**context):
    ...
    try:
        ...
        transformed_data_json = {k: v.to_json(orient='split') for k, v in transformed_data.items()}
        context['ti'].xcom_push(key='misa_crm_transformed_data', value=json.dumps(transformed_data_json))
...
def load_misa_crm_data(**context):
    ...
    transformed_data_json = context['ti'].xcom_pull(key='misa_crm_transformed_data', task_ids='misa_crm_etl.transform')
    transformed_data_dict = json.loads(transformed_data_json)

    # Deserialize JSON back to DataFrames
    transformed_data = {k: pd.read_json(v, orient='split') for k, v in transformed_data_dict.items()}
...
def extract_tiktok_shop_full_load(**context):
    ...
    try:
        ...
        if start_date is None:
            ...
            logger.info(f"✅ Auto-detected start date: {start_date.strftime('%Y-%m-%d')}")
        ...
        logger.info(f"📅 Processing range: {start_date.strftime('%Y-%m-%d')} to {end_date.strftime('%Y-%m-%d')} ({days_span} days)")
        ...
        for orders_batch in extractor.stream_orders_lightweight(start_timestamp, end_timestamp, batch_size=20):
            batch_count += 1
            batch_size = len(orders_batch)
            
            if orders_batch:
                all_orders.extend(orders_batch)
                logger.info(f"📥 Batch {batch_count}: extracted {batch_size} orders (total: {len(all_orders)})")
        ...
        logger.info(f"✅ Extraction complete: {len(all_orders)} orders from {batch_count} batches")
        
        # Push to XCom
        context['ti'].xcom_push(key='tiktok_shop_raw_data', value=all_orders)
        context['ti'].xcom_push(key='extraction_metadata', value={
...
def transform_tiktok_shop_full_load(**context):
    ...
    try:
        ...
        # Push to XCom
        context['ti'].xcom_push(key='tiktok_shop_transformed_data', value=transformed_data)
...
def extract_shopee_orders_full_load(**context):
    ...
    try:
        ...
        # Push to XCom
        context['ti'].xcom_push(key='shopee_orders_raw_data', value=json.dumps(all_orders))
...
def transform_shopee_orders_full_load(**context):
    ...
    try:
        ...
        # Push to XCom
        context['ti'].xcom_push(key='shopee_orders_transformed_data', value=json.dumps(transformed_data))
...
def load_shopee_orders_full_load(**context):
    ...
    try:
        # Pull transformed data from XCom
        transformed_data_json = context['ti'].xcom_pull(key='shopee_orders_transformed_data')
