...
class Settings:
    ...
    def __init__(self):
        ...
        # DATABASE SETTINGS
        # ========================
        self.sql_server_host: str = os.getenv('SQL_SERVER_HOST', 'sqlserver') # Use the service name from docker-compose for container-to-container communication
        self.sql_server_port: int = int(os.getenv('SQL_SERVER_PORT', '1433'))
        self.sql_server_database: str = os.getenv('SQL_SERVER_DATABASE', 'Facolos_Staging')
        ...
        self.staging_table_orders: str = 'tiktok_shop_order_detail'  # Fixed: Use correct table name
        ...
        self.misa_crm_max_page_size: int = 5000000  # Max records per API call for full load

        self.misa_crm_token_refresh_buffer: int = int(os.getenv('MISA_CRM_TOKEN_REFRESH_BUFFER', '300')) # 5 minutes
        self.misa_crm_etl_batch_size: int = int(os.getenv('MISA_CRM_ETL_BATCH_SIZE', '1000'))
        self.misa_crm_incremental_lookback_hours: int = int(os.getenv('MISA_CRM_INCREMENTAL_LOOKBACK_HOURS', '24'))
        ...
        self.shopee_token_refresh_buffer: int = int(os.getenv('SHOPEE_TOKEN_REFRESH_BUFFER', '300'))  # 5 minutes
        self.shopee_etl_batch_size: int = int(os.getenv('SHOPEE_ETL_BATCH_SIZE', '1000'))
        self.shopee_incremental_lookback_minutes: int = int(os.getenv('SHOPEE_INCREMENTAL_LOOKBACK_MINUTES', '15'))
        # ========================
        # API SETTINGS
        # ========================
        ...
        self.etl_default_frequency_hours: float = float(os.getenv('ETL_DEFAULT_FREQUENCY_HOURS', '6'))

        # Standardized incremental lookback windows (đồng nhất 15 phút)
        self.etl_incremental_lookback_minutes: int = int(os.getenv('ETL_INCREMENTAL_LOOKBACK_MINUTES', '15'))
        
        # ========================
        # TEST ETl SETTINGS
        # ========================
        self.test_etl_days_back: int = int(os.getenv('TEST_ETL_DAYS_BACK', '2'))  # Test với 2 ngày gần nhất
        self.test_etl_max_orders: int = int(os.getenv('TEST_ETL_MAX_ORDERS', '50'))  # Test tối đa 50 orders
        self.test_etl_max_records_per_endpoint: int = int(os.getenv('TEST_ETL_MAX_RECORDS_PER_ENDPOINT', '5'))  # Test 5 records per endpoint
    ...
    def get_active_data_sources(self) -> List[str]:
        ...
        # Check MISA CRM
        if self.misa_crm_client_id and self.misa_crm_client_secret and self.misa_crm_access_token:
            active_sources.append('misa_crm')
            
        # Check Shopee
        if self.shopee_partner_id and self.shopee_partner_key and self.shopee_shop_id and self.shopee_access_token:
    ...
    def get_env_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'misa_crm_configured': bool(self.misa_crm_client_id and self.misa_crm_client_secret and self.misa_crm_access_token),
            'shopee_configured': bool(self.shopee_partner_id and self.shopee_partner_key and self.shopee_shop_id and self.shopee_access_token)
        }
