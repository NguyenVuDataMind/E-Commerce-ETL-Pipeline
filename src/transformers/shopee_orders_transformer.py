...
class ShopeeOrderTransformer:
    ...
    def transform_orders_to_dataframes(self, orders_list: List[Dict[str, Any]]) -> Dict[str, pd.DataFrame]:
        """
    ...
    def _process_single_order(self, order: Dict[str, Any], dataframes: Dict[str, List]):
        ...
        self._process_order_items(order, dataframes['order_items'], dataframes['order_item_locations'])
        
        # 4. Packages and package items
        self._process_packages(order, dataframes['packages'], dataframes['package_items'])
    ...
    def _process_orders_table(self, order: Dict[str, Any], orders_list: List):
        ...
        order_data = {
            ...
            'estimated_shipping_fee': self._safe_float(order.get('estimated_shipping_fee')),
            ...
            'actual_shipping_fee_confirmed': self._safe_bool(order.get('actual_shipping_fee_confirmed')),
            ...
            'order_chargeable_weight_gram': self._safe_int(order.get('order_chargeable_weight_gram')),
            'prescription_check_status': self._safe_int(order.get('prescription_check_status')),
            'pharmacist_name': order.get('pharmacist_name'),
            'prescription_approval_time': self._unix_to_datetime(order.get('prescription_approval_time')),
            'prescription_rejection_time': self._unix_to_datetime(order.get('prescription_rejection_time')),
            ...
            'return_request_due_date': self._unix_to_datetime(order.get('return_request_due_date')),
            'is_buyer_shop_collection': self._safe_bool(order.get('is_buyer_shop_collection'))
        }
    ...
    def _process_recipient_address(self, order: Dict[str, Any], address_list: List):
        ...
        address_data = {
            ...
            'latitude': self._safe_float(recipient_address.get('geolocation', {}).get('latitude')) if recipient_address.get('geolocation') else None,
            'longitude': self._safe_float(recipient_address.get('geolocation', {}).get('longitude')) if recipient_address.get('geolocation') else None
        }
        address_list.append(address_data)
    
    def _process_order_items(self, order: Dict[str, Any], items_list: List, locations_list: List):
        ...
        for item in item_list:
            ...
            item_data = {
                ...
                'model_quantity_purchased': self._safe_int(item.get('model_quantity_purchased')),
                'model_original_price': self._safe_float(item.get('model_original_price')),
                'model_discounted_price': self._safe_float(item.get('model_discounted_price')),
                ...
                'is_prescription_item': self._safe_bool(item.get('is_prescription_item')),
    ...
    def _process_packages(self, order: Dict[str, Any], packages_list: List, package_items_list: List):
        ...
        for package in package_list:
            ...
            package_data = {
                ...
                'logistics_channel_id': self._safe_int(package.get('logistics_channel_id')),
                'shipping_carrier': package.get('shipping_carrier'),
                'allow_self_design_awb': self._safe_bool(package.get('allow_self_design_awb')),
                'parcel_chargeable_weight_gram': self._safe_int(package.get('parcel_chargeable_weight_gram')),
            ...
            for pkg_item in package_item_list:
                package_items_list.append({
                    ...
                    'model_quantity': self._safe_int(pkg_item.get('model_quantity')),
                    'promotion_group_id': self._safe_int(pkg_item.get('promotion_group_id')),
                    'product_location_id': pkg_item.get('product_location_id'),
                    'parcel_chargeable_weight': self._safe_int(pkg_item.get('parcel_chargeable_weight'))
                })
    ...
    def _process_invoice(self, order: Dict[str, Any], invoice_list: List):
        ...
        invoice_data = {
            ...
            'products_total_value': self._safe_float(invoice_data_obj.get('products_total_value')),
    ...
    def transform_orders_to_flat_dataframe(self, orders_list: List[Dict[str, Any]]) -> pd.DataFrame:
        """
        ...
        for order in orders_list:
            ...
            if recipient_address:
                order_data.update({
                    ...
                    'recipient_latitude': self._safe_float(recipient_address.get('geolocation', {}).get('latitude')) if recipient_address.get('geolocation') else None,
                    'recipient_longitude': self._safe_float(recipient_address.get('geolocation', {}).get('longitude')) if recipient_address.get('geolocation') else None
                })
            ...
            if item_list:
                ...
                order_data.update({
                    ...
                    'model_quantity_purchased': self._safe_int(first_item.get('model_quantity_purchased')),
                    'model_original_price': self._safe_float(first_item.get('model_original_price')),
                    'model_discounted_price': self._safe_float(first_item.get('model_discounted_price')),
                    'item_weight': self._safe_float(first_item.get('weight')),
                    'item_image_url': first_item.get('image_info', {}).get('image_url') if first_item.get('image_info') else None
                })
            ...
            if package_list:
                ...
                order_data.update({
                    ...
                    'package_logistics_status': first_package.get('logistics_status'),
                    'package_shipping_carrier': first_package.get('shipping_carrier'),
                    'package_logistics_channel_id': self._safe_int(first_package.get('logistics_channel_id'))
                })
            ...
            if invoice_data:
                order_data.update({
                    ...
                    'invoice_issue_date': self._unix_to_datetime(invoice_data.get('issue_date')),
                    'invoice_total_value': self._safe_float(invoice_data.get('total_value')),
                    'invoice_products_total_value': self._safe_float(invoice_data.get('products_total_value')),
