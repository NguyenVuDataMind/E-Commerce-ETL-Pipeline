...
class TikTokShopOrderTransformer:
    ...
    def transform_orders_to_dataframe(self, orders: List[Dict[str, Any]], 
        ...
        try:
            ...
            for i in range(0, len(orders), chunk_size):
                ...
                logger.info(f"Processing transformation chunk {chunk_num}/{total_chunks}: {len(chunk_orders)} orders")
            ...
            logger.info(f"Transformation completed. Final DataFrame shape: {final_df.shape}")
            return final_df
    ...
    def _extract_order_info(self, order: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            'payment_method_name': self._safe_string(order.get('payment_method_name'), 200),
            ...
            'delivery_option_id': self._safe_string(order.get('delivery_option_id'), 100),
            'delivery_option_name': self._safe_string(order.get('delivery_option_name'), 200),
            'delivery_type': self._safe_string(order.get('delivery_type'), 100),
            'fulfillment_priority_level': self._safe_int(order.get('fulfillment_priority_level')),
            'has_updated_recipient_address': self._safe_bool(order.get('has_updated_recipient_address')),
            ...
            'recommended_shipping_time': self._safe_int(order.get('recommended_shipping_time')),
            ...
            'shipping_provider_id': self._safe_string(order.get('shipping_provider_id'), 100),
            ...
            'is_buyer_request_cancel': self._safe_bool(order.get('is_buyer_request_cancel')),
            'cancel_reason': order.get('cancel_reason'),
            'cancel_user': self._safe_string(order.get('cancel_user'), 100),
            'buyer_uid': self._safe_string(order.get('buyer_uid'), 100),
            'split_or_combine_tag': self._safe_string(order.get('split_or_combine_tag'), 100),
            ...
            'payment_original_shipping_fee': self._safe_decimal(payment_info.get('original_shipping_fee')),
            'payment_original_total_product_price': self._safe_decimal(payment_info.get('original_total_product_price')),
            'payment_platform_discount': self._safe_decimal(payment_info.get('platform_discount')),
            'payment_seller_discount': self._safe_decimal(payment_info.get('seller_discount')),
            'payment_shipping_fee': self._safe_decimal(payment_info.get('shipping_fee')),
            'payment_shipping_fee_cofunded_discount': self._safe_decimal(payment_info.get('shipping_fee_cofunded_discount')),
            'payment_shipping_fee_platform_discount': self._safe_decimal(payment_info.get('shipping_fee_platform_discount')),
            'payment_shipping_fee_seller_discount': self._safe_decimal(payment_info.get('shipping_fee_seller_discount')),
            'payment_sub_total': self._safe_decimal(payment_info.get('sub_total')),
            'payment_tax': self._safe_decimal(payment_info.get('tax')),
            'payment_total_amount': self._safe_decimal(payment_info.get('total_amount')),
            ...
            'package_shipping_provider_id': self._safe_string(package.get('shipping_provider_id'), 100),
            'package_shipping_provider_name': self._safe_string(package.get('shipping_provider_name'), 200),
            'package_tracking_number': self._safe_string(package.get('tracking_number'), 200),
    ...
    def _extract_recipient_info(self, order: Dict[str, Any]) -> Dict[str, Any]:
        ...
        district_info_json = json.dumps(recipient.get('district_info', [])) if recipient.get('district_info') else None
        
        return {
            # FIXED: Recipient Address Fields - Map theo JSON structure thực tế
            'recipient_address_detail': recipient.get('address_detail'),  # FIXED: từ 'detail' thành 'address_detail'
            ...
            'recipient_first_name_local_script': recipient.get('first_name_local_script'),
    ...
    def _extract_item_info(self, item: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            'item_id': str(item.get('id', '')),  # FIXED: sử dụng 'id' thay vì 'product_id'
            'item_product_id': item.get('product_id'),
            'item_product_name': item.get('product_name'),
            'item_sku_id': item.get('sku_id'),
            'item_sku_name': item.get('sku_name'),  # FIXED: trực tiếp từ item, không có sku_info
            ...
            'item_is_buyer_request_cancel': self._safe_bool(item.get('is_buyer_request_cancel')),
            ...
            'item_sku_attributes': json.dumps(item) if item else None  # Store toàn bộ item data
        }
