...
class ShopeeOrderExtractor:
    ...
    def _load_persistent_tokens(self):
        ...
        if self.access_token:
            logger.warning("⚠️ No Shopee token found in DB. Using .env temporarily for first run.")
        else:
            logger.warning(
                "⚠️ Chưa có Shopee access_token trong DB và .env. Hãy thêm SHOPEE_ACCESS_TOKEN/SHOPEE_REFRESH_TOKEN vào .env cho lần đầu."
            )
    
    def create_signature(self, path: str, timestamp: int, access_token: str = None, shop_id: str = None) -> str:
        """
    ...
    def get_access_token_from_code(self, authorization_code: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if response.status_code == 200 and not response_json.get('error'):
                ...
                self.token_expires_at = datetime.now() + timedelta(seconds=response_json.get('expire_in', 14400))
                
                logger.info("✅ Successfully obtained access token")
                return response_json
            else:
                logger.error(f"❌ Failed to get access token: {response_json.get('error')} - {response_json.get('message')}")
                return None
    ...
    def refresh_access_token(self) -> bool:
        ...
        try:
            ...
            if response.status_code == 200 and not response_json.get('error'):
                ...
                logger.error(f"❌ Failed to refresh token: {response_json.get('error')} - {response_json.get('message')}")
                return False
    ...
    def get_order_list(self, time_from: int, time_to: int, page_size: int = 100, 
                      time_range_field: str = "create_time") -> Optional[Dict[str, Any]]:
        """
        ...
        try:
            ...
            if response.status_code != 200 or data.get('error'):
                logger.error(f"❌ Error getting order list: {data.get('error')} - {data.get('message')}")
                return None
            
            logger.info(f"✅ Retrieved {len(data.get('response', {}).get('order_list', []))} orders")
            return data
    ...
    def get_order_detail(self, order_sn_list: List[str]) -> Optional[Dict[str, Any]]:
        ...
        if len(order_sn_list) > 50:
            logger.warning(f"⚠️ Order list too long ({len(order_sn_list)}), truncating to 50")
        ...
        try:
            ...
            if response.status_code != 200 or data.get('error'):
                logger.error(f"❌ Error getting order detail: {data.get('error')} - {data.get('message')}")
                return None
            
            logger.info(f"✅ Retrieved details for {len(data.get('response', {}).get('order_list', []))} orders")
            return data
    ...
    def extract_orders_full_load(self, start_date: datetime, end_date: datetime) -> List[Dict[str, Any]]:
        """
        ...
        """
        logger.info(f"🚀 Starting Shopee full load extraction from {start_date} to {end_date}")
        ...
        while True:
            ...
            order_sn_list = [order.get('order_sn') for order in order_list if order.get('order_sn')]
            ...
            if order_sn_list:
                ...
                if detail_response:
                    orders_detail = detail_response.get('response', {}).get('order_list', [])
                    all_orders.extend(orders_detail)
                    logger.info(f"✅ Processed {len(orders_detail)} orders (Total: {len(all_orders)})")
                else:
    ...
    def extract_orders_incremental(self, minutes_back: int = 15) -> List[Dict[str, Any]]:
        """
        ...
        """
        logger.info(f"🔄 Starting Shopee incremental extraction: {minutes_back} minutes back")
        ...
        for i in range(0, len(order_sn_list), batch_size):
            ...
            if detail_response:
                orders_detail = detail_response.get('response', {}).get('order_list', [])
                all_orders.extend(orders_detail)
                logger.info(f"✅ Processed batch {i//batch_size + 1}: {len(orders_detail)} orders")
            else:
        ...
        logger.info(f"🎉 Incremental extraction completed: {len(all_orders)} new orders")
        return all_orders
    ...
    def find_earliest_order_date(self, max_lookback_years: int = 2) -> Optional[datetime]:
        ...
        """
        logger.info(f"🔍 Auto-detecting earliest order date (max {max_lookback_years} years back)")
