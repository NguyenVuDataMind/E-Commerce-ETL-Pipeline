...
class ShopeeOrderExtractor:
    ...
    def get_order_list(
        ...
        try:
            ...
            logger.info(
                f"✅ Retrieved {len(data.get('response', {}).get('order_list', []))} orders"
            )
            return data
    ...
    def get_order_detail(self, order_sn_list: List[str]) -> Optional[Dict[str, Any]]:
        ...
        if len(order_sn_list) > 50:
            logger.warning(
                f"⚠️ Order list too long ({len(order_sn_list)}), truncating to 50"
            )
        ...
        try:
            ...
            logger.info(
                f"✅ Retrieved details for {len(data.get('response', {}).get('order_list', []))} orders"
            )
            return data
